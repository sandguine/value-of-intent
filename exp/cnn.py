""" 
Based on PureJaxRL Implementation of PPO with CNN
"""
...
class CNN(nn.Module):
    """Modular CNN for feature extraction in PPO."""
    activation: str = "tanh"

    def setup(self):
        """Define named layers for clarity and maintainability."""
        self.act_fn = nn.relu if self.activation == "relu" else nn.tanh

        # Convolutional layers for spatial feature extraction
        self.conv_spatial_1 = nn.Conv(
            features=32, kernel_size=(5, 5),
            kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0),
            name="conv_spatial_1"
        )
        self.conv_spatial_2 = nn.Conv(
            features=32, kernel_size=(3, 3),
            kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0),
            name="conv_spatial_2"
        )
        self.conv_spatial_3 = nn.Conv(
            features=32, kernel_size=(3, 3),
            kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0),
            name="conv_spatial_3"
        )

        # Fully connected (dense) layer for final feature aggregation
    ...
    @nn.compact
    def __call__(self, x):
        """Forward pass of CNN with named layers."""
        x = self.conv_spatial_1(x)
        x = self.act_fn(x)
        x = self.conv_spatial_2(x)
        x = self.act_fn(x)
        x = self.conv_spatial_3(x)
        x = self.act_fn(x)

        # Flatten spatial dimensions before dense layer
...
class ActorCritic(nn.Module):
    """Actor-Critic network for PPO using CNN-based feature extraction."""
    action_dim: int  # Number of possible actions
    activation: str = "tanh"

    def setup(self):
        """Define named layers for modularity and readability."""
        self.act_fn = nn.relu if self.activation == "relu" else nn.tanh

        # Feature extraction via CNN
        self.feature_extractor = CNN(activation=self.activation)
    ...
    def __call__(self, x):
        ...
        # Actor head: Computes action logits
        ...
        # Critic head: Computes state value
